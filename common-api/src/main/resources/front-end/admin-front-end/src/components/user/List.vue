<template>
  ...
  >
    <van-cell v-for="item in list" :key="item" :title="item.userName" :to="'/um/' + item.userId"/>
  </van-list>
  <!--    </van-pull-refresh>-->
  <!--    回到顶部-->
...
export default {
  ...
  setup() {
    ...
    const getTotalCount = () => {
      // 获取最大记录
      axios.get('/user-service/system/user/admin/getUserTotal').then(res => {
    ...
    const onLoad = () => {
      ...
      setTimeout(() => {
        ...
        // 拼接url
        axios.get('/user-service/system/user/admin/getAllUser/' + page.value + '/' + limit.value).then(res => {
