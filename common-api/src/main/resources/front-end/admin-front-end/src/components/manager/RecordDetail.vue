...
export default {
  ...
  setup() {
    ...
    axios.get('/event-service/system/event/admin/getUserCafeDetailRecord/' + router.currentRoute.value.params.id)
        .then(res => {
    ...
    const updateBy = ref(JSON.parse(useUserStore().getUser).userId)
    const onSubmit = (values) => {
      axios({
        method: 'put',
        url: '/event-service/system/event/updateRecord',
        data: {
          id: tid.value,
          sponsorId: uid.value,
    ...
    const onClickRight = () => {
      axios.delete('/event-service/system/event/admin/delUserApplyById/' + router.currentRoute.value.params.id).then(res => {
        if (res.data.code === 200) {
          showNotify({type: "success", message: res.data.message});
          router.replace('/am');
        } else if (res.data.code === 500) {
