...
export default {
  ...
  setup() {
    ...
    const userInfo = ref(useUserStore().getUser);
    const userName = ref(useUserStore().getUser.userName);
    const uid = ref(useUserStore().getUser.id)
    const password = ref('')
    const email = ref(useUserStore().getUser.email)
    const profession = ref(useUserStore().getUser.profession)
    const ress = ref([
    ...
    const onSubmit = () => {
      axios({
        method: "put",
        url: '/user-service/system/user/putUserInfo',
        data: {
          id: uid.value,
