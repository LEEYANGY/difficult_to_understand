...
export default {
  ...
  setup() {
    ...
    const getTotalCount = () => {
      // 获取最大记录
      axios.get('/user-service/system/user/getMyZoneTotal/' + JSON.parse(useUserStore().getUser).userId).then(res => {
    ...
    const onLoad = () => {
      ...
      setTimeout(() => {
        ...
        // 拼接url
        axios.get('/user-service/system/user/getMyZone/' + +JSON.parse(useUserStore().getUser).userId + '/' + page.value + '/' + 10).then(res => {
