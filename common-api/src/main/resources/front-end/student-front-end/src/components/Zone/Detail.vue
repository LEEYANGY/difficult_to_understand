...
export default {
  ...
  setup() {
    ...
    const onClickRight = () => {
      console.log('6666     '+sponsorBy.value, JSON.parse(store.getUser).userId)
      if (JSON.parse(store.getUser).userId.toString() === sponsorBy.value) {
        // 路由传参
    ...
    axios.get('/zone-service/system/zone/getDetail/' + router.currentRoute.value.params.id).then(res => {
      if (res.data.code === 200) {
        ...
        sponsorBy.value = res.data.data.sponsorId
