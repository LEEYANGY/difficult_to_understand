...
export default {
  ...
  setup() {
    ...
    const getTotalCount = () => {
      // 获取最大记录
      axios.get('system/event/getTotal/' + JSON.parse(useUserStore().getUser)[0].userId).then(res => {
    ...
    const onLoad = () => {
      ...
      setTimeout(() => {
        ...
        // 拼接url
        axios.get('system/event/getMyCafeRecord/' + JSON.parse(useUserStore().getUser)[0].userId + '/' + page.value + ' / ' + 10).then(res => {
