...
export default {
  ...
  setup() {
    ...
    const onSubmit = (values) => {
      ...
      axios({
        method: 'post',
        url: 'system/zone/editZone',
        data: {
          sponsorId: JSON.parse(useUserStore().getUser)[0].userId,
          sponsor: JSON.parse(useUserStore().getUser)[0].userName,
          id: id.value,
          title: title.value,
          content: content.value,
          remark: remark.value,
          createBy: JSON.parse(useUserStore().getUser)[0].userId,
          updateBy: JSON.parse(useUserStore().getUser)[0].userId,
        }
      }).then(res => {
        if (res.data.code === 200) {
    ...
    const delZone = () => {
      axios.delete('/system/zone/delZone/' + router.currentRoute.value.params.id).then(res => {
        if (res.data.code === 200) {
